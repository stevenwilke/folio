// Fiction genres (must match canonical names from src/lib/genres.js)
...
export function isFiction(genre) {
 ...
 */
export function computeReadingSpeeds(sessions) {
  const fictionSpeeds = []
  const nonfictionSpeeds = []
  ...
  for (const s of sessions) {
    ...
    // Filter out bad data: too short or too long
    ...
    // null is_fiction goes only into blended
  }
 ...
 * @param {number} currentPage - current page (0 for unstarted)
 * @param {string|null} genre - book genre
 * @param {{ fiction, nonfiction, blended }} speeds - from computeReadingSpeeds
 * @returns {{ minutes: number, label: string } | null}
 */
export function estimateReadingTime(totalPages, currentPage, genre, speeds) {
  ...
  const speed = fic === true ? (speeds.fiction || speeds.blended)
...
export function checkSessionIdle(startedAt, thresholdMin = 30) {
